...
class QLearning():
    ...
    def initialize_policy(self):
        ...
        keys = list(itertools.product(l, repeat=Constants.UNIT_SIZE))
    ...
    def select_action(self, key):
        ...
        if (x > self.epsilon):
            choice = int(self.policy[key].index(
                (random.choice(self.policy[key]))))
        else:
    ...
    def export_policy(self, file_name):
        with open(file_name, 'w') as f:
            for key in self.policy.keys():
                item = "{}:{}:{}".format(
                    key, self.policy[key][0], self.policy[key][1])
                f.write("%s\n" % item)
...
def closest_unit(num):
    temp = float('%.1f' % ((num-Constants.SPAWN) /
                           (Constants.WINDOW_WIDTH - Constants.SPAWN)))
    if(temp == 1):
        temp = 0.9
    # convert to string to remove floating point errors
    index_finder = [float('%.1f' % (i*(1/Constants.UNIT_SIZE)))
                    for i in range(Constants.UNIT_SIZE)]
